...
if(ex){var chips=[].slice.call(ex.querySelectorAll('.filter-chip')),q=ex.querySelector('input[type=search]'),empty=ex.querySelector('[data-empty]'),cur={},list=ex.querySelector('.rows'),initial=list.innerHTML,index,request=0,pager=ex.querySelector('.pager'),base=ex.getAttribute('data-facet'),rising=ex.hasAttribute('data-rising');
async function apply(){var ticket=++request,s=(q&&q.value||'').trim().toLowerCase(),active=Object.keys(cur).some(function(k){return !!cur[k];});
if(!s&&!active){list.innerHTML=initial;if(empty)empty.hidden=!!list.children.length;if(pager)pager.hidden=false;return;}
...
if(rising)matches.sort(function(a,b){return b.starsDelta24h-a.starsDelta24h;});list.innerHTML=matches.map(function(it){return rising?it.risingHtml:it.html;}).join('');if(empty)empty.hidden=matches.length>0;if(pager)pager.hidden=true;}catch(e){}}
